...
import {
  fetchAnimalProductsAsync,
  selectAnimal,
} from "../redux/slice/fetchAnimalProducts";
...
const DailyPriceList = () => {
  ...
  useEffect(() => {
    try {
      setTotalPages(Math.ceil(Animal.length / itemsPerPage));
    } catch (error) {
  ...
  const endItem = Math.min(startItem + itemsPerPage, Animal.length);
    ...
    <div>
      <TableContainer component={Paper} className="table-container">
        <Table className="table">
          ...
          <TableBody>
            {Animal.slice(startItem, endItem).map((price, index) => (
